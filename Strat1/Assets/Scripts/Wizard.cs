    ...
    void FixedUpdate()
    {
        ...
        if(!currentAnim.IsName("Attack"))
        {
            if(Input.GetKey(KeyCode.LeftArrow)&&!onLadder)
            {
                ...
                transform.localScale = reverse_direction*character_proportion;
            ...
            {
                transform.Translate(speed*Time.deltaTime,0,0);
                transform.localScale = standard_direction*character_proportion;
                anim.SetBool("isRun", true);
                if(!isGrounded)
                {
                    anim.SetBool("isRun", false);
    ...
    void downLadder()
    {
     ...
     private void OnTriggerEnter2D(Collider2D other) {
        if(other.CompareTag("Ladder"))
        {
            ...
            Debug.Log(other.bounds.min);
            canLadder = true;
            ladderTop = new Vector2(other.bounds.center.x,other.bounds.max.y);
            ladderBottom = new Vector2(other.bounds.center.x,other.bounds.min.y);
        }
